...
export default {
        ...
        options: {
            ...
            {formatter:"rowSelection", titleFormatter:"rowSelection", align:"center", headerSort:false, width: 30},    
            {
                ...
                formatter:function(cell){
                    ...
                    if (showSendSms) {
                        cell_html += "<a href='javascript: void(0);' class='lfp-clickable-number' data-number='" + cell.getRow().getData().international_phone_number + "' data-search-id='" + cell.getRow().getData().id + "' style='font-weight: bold; margin-top: 10px; font-size: 15px !important;'><i class='fa fa-sms' style='font-size: 16px !important;'></i> " + cell.getRow().getData().formatted_phone_number + "</a>";
                    }
                    else {
  ...
  methods: {
    ...
    async processSendBulk() {
        ...
        await axios.post(process.env.VUE_APP_API_ENDPOINT + '/inbox/send-bulk', {
