...
export default {
  ...
  methods: {
    ...
    async deleteReviewplatform(listing_id) {
      ...
      for (var i6 in this.listings) {
        let listing = this.listings[i6];
    ...
    async submitNapScan() {
      ...
      try {
        await axios.post(process.env.VUE_APP_API_ENDPOINT + '/me/listings/submit-nap', {
    ...
    async dismissListingsSearch() {
      ...
      try {
         await axios.post(process.env.VUE_APP_API_ENDPOINT + '/me/listings/dismiss-nap', {
        } , {
          headers: {
            'Authorization': 'Bearer ' + this.$store.state.auth.jwt
          },
  ...
  created() {
    ...
    for (var i2 in this.listings) {
      if (this.listings[i2].crawl_status == "pending" || !this.listings[i2].crawl_status)
        hasPending = true;
    }
  ...
  watch: {
    listings: function() {
      this.active_connections = {};
      for (var i3 in this.listings) {
        let listing = this.listings[i3];
        let hostName = this.getHostname(listing.url);
      ...
      this.reviewSitesNamesCount = 0;
      for (var i4 = 0; i4 < this.reviewSitesNamesWidgetsOnly.length; i4++)
        this.reviewSitesNamesCount++;

      this.reviewSitesNamesWidgetsOnlyCount = 0;
      for (var i5 = 0; i5 < this.reviewSitesNamesWidgetsOnly.length; i5++)
