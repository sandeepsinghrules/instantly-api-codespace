...
export default {
  ...
  methods: {
    ...
    saveFunnel: async function() {
		...
		let self = this
		try {
			await axios.post(process.env.VUE_APP_API_ENDPOINT + '/review-funnels/save-funnel', {
    ...
    setCustomCSS() {
        ...
        if (this.review_funnel.custom_css) {
            ...
            if (document.getElementById("rgrf-custom-css-sheet") === null) {
                ...
                styleSheet2.id = 'rgrf-custom-css-sheet';
    ...
    deleteSource(index) {
        ...
        for (var i2 in new_sources)
            this.review_funnel.sources.push(new_sources[i2]);
    },
