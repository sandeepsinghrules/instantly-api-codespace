        ...
        <v-stepper-items>
          ...
          <v-stepper-content step="2">
            ...
            <div class="flex flex-wrap -mx-3 -mt-4">
              <div :class="'w-full ' + ((only_business) ? 'md:w-1/2 px-3' : 'pl-3') + ' mb-4 md:mb-0'">
                <br v-if="company_type != 'business'" />
                <p class="mb-1 text-xs mb-2">
                  Company Name
                </p>
...
export default {
  ...
  methods: {
    async submit(isIgnoreOtp) {
      ...
      try {
        ...
        if (this.company_type == 'agency')
          params.subdomain = this.$refs.subdomain.value.trim()
        ...
        let response = await axios.post(`${process.env.VUE_APP_API_ENDPOINT}/users/register`, params)
        ...
        if (!response.data.success) {
          ...
          this.$store.dispatch('setPassword',params.password );
          // params.password = undefined;
          // console.log(data.user);
          this.$store.dispatch('setJwtToken',JSON.stringify(response.data.token));
          this.$store.dispatch('setUser',JSON.stringify(response.data.user));
          this.$store.dispatch('dashboardapex/setDashboardGraph',response.data.user.id);
        ...
        //     first_name: self.$refs.first_name.value.trim(),
        //     last_name: self.$refs.last_name.value.trim(),
    ...
    selectPlace() {
      ...
      $.get("https://maps.googleapis.com/maps/api/geocode/json?place_id=" + this.autocompleteLocationModel.id + "&fields=name,rating,formatted_phone_number&key=AIzaSyC4XJt0FM5BNTS7c6ur4kf_bXJHMSIvowg", function(response) {
